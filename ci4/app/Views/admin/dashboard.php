...
<body class="bg-slate-50 flex h-screen overflow-hidden">
    ...
    <?php
        ...
        $adminRoleIds = ['12', '13', '14', '15', '27', '28', '29', '30', 'Admin'];
        $isAdmin = in_array($userRoleId, $adminRoleIds);
    ...
    <aside class="w-64 bg-emerald-900 text-white flex flex-col hidden md:flex shadow-xl z-20">
        <div class="h-20 flex items-center justify-center border-b border-emerald-800">
            <img src="<?= base_url('logo/denr_logo.png') ?>" alt="Logo" class="h-10 w-10 mr-3">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto" id="sidebar-menu">
            ...
            <a href="<?= base_url('/admin/applications') ?>" target="content-frame" onclick="showIframe(this, 'Applications');" class="nav-link flex items-center gap-3 px-4 py-3 text-emerald-100 hover:bg-emerald-800 hover:text-white rounded-xl transition">
            ...
            <a href="<?= base_url('/admin/manage-users') ?>" target="content-frame" onclick="showIframe(this, 'Manage Users');" class="nav-link flex items-center gap-3 px-4 py-3 text-emerald-100 hover:bg-emerald-800 hover:text-white rounded-xl transition">
                <i class="fas fa-users w-5"></i> Manage Users
            </a>
            
            <?php if ($userRoleId === '15' || strtolower($userRoleId) === 'admin'): ?>
            <a href="<?= base_url('/admin/manage-requirements') ?>" target="content-frame" onclick="showIframe(this, 'Manage Requirements');" class="nav-link flex items-center gap-3 px-4 py-3 text-emerald-100 hover:bg-emerald-800 hover:text-white rounded-xl transition">
        ...
        <div class="p-4 border-t border-emerald-800">
            <a href="<?= base_url('/logout') ?>" class="flex items-center gap-3 px-4 py-3 text-red-300 hover:bg-red-900 hover:text-white rounded-xl transition">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <div class="flex items-center gap-6">
                            ...
                            <?php if ($notificationCount > 0): ?>
                                <ul class="divide-y divide-gray-50">
                                    <?php foreach ($notifications as $notif): ?>
                                        <li class="p-4 hover:bg-gray-50 transition cursor-pointer" onclick="document.querySelector('a[href\="<?= base_url('/admin/applications') ?>"]').click(); toggleNotifications();">
                ...
                <div class="flex items-center gap-3 pl-6 border-l border-gray-200">
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-bold text-gray-800"><?= htmlspecialchars($userName ?? 'User') ?></p>
                        <p class="text-xs text-emerald-600 font-bold uppercase tracking-wider"><?= htmlspecialchars($userRoleName ?? 'Staff') ?></p>
                    </div>
                    <div class="h-10 w-10 rounded-full <?= $isAdmin ? 'bg-purple-100 text-purple-700 border-purple-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200' ?> flex items-center justify-center text-lg border shadow-sm">
                        <i class="fas <?= $isAdmin ? 'fa-user-cog' : 'fa-user-shield' ?>"></i>
                    </div>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 relative flex flex-col min-h-0 bg-slate-50 z-0">
            ...
            <div id="dashboard-content" class="absolute inset-0 overflow-y-auto p-8 fade-in">
                <div class="mb-8">
                    <h1 class="text-3xl font-extrabold text-gray-900">Welcome back, <?= htmlspecialchars($userName ?? 'User') ?>! 👋</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-6 hover:shadow-md transition">
                        ...
                        <div>
                            ...
                            <h3 class="text-3xl font-black text-gray-900 mt-1"><?= number_format($totalApps ?? 0) ?></h3>
                        </div>
                    </div>

                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-6 hover:shadow-md transition cursor-pointer" onclick="document.querySelector('a[href\="<?= base_url('/admin/applications') ?>"]').click();">
                        ...
                        <div>
                            ...
                            <h3 class="text-3xl font-black text-gray-900 mt-1"><?= number_format($pendingApps ?? 0) ?></h3>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-6 hover:shadow-md transition">
                        ...
                        <div>
                            ...
                            <h3 class="text-3xl font-black text-gray-900 mt-1"><?= number_format($approvedApps ?? 0) ?></h3>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        ...
                        <a href="#" onclick="document.querySelector('a[href\="<?= base_url('/admin/applications') ?>"]').click();" class="text-sm bg-white border border-gray-200 px-4 py-2 rounded-lg text-emerald-700 font-semibold hover:bg-emerald-50 transition shadow-sm">View All</a>
                    </div>
