...
<head>
    ...
    <title><?= $appType ?> Application | O-LDPMS</title>
...
<body class="bg-slate-50 min-h-screen pb-20">
    ...
    <nav class="bg-emerald-900 text-white h-16 flex items-center justify-between px-6 sticky top-0 z-50 shadow-md">
        <a href="<?= base_url('/client/dashboard') ?>" target="_top" class="flex items-center gap-2 text-emerald-200 hover:text-white font-semibold transition">
        ...
        <div class="font-black tracking-tight text-lg uppercase flex items-center gap-3">
            <img src="<?= base_url('logo/denr_logo.png') ?>" class="h-8 w-8" alt="Logo">
            O-LDPMS <span class="text-emerald-400 font-medium text-sm hidden sm:inline">| <?= $appType ?> Application</span>
        </div>
        <div class="w-24 sm:w-32"></div> 
    </nav>

    <div class="max-w-5xl mx-auto mt-10 px-4 mb-6 text-center">
        <h2 class="text-3xl font-extrabold text-gray-900">Create <?= $appType ?> Application</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" readonly value="<?= esc($user['firstname'] . ' ' . $user['mid_name'] . ' ' . $user['lastname']) ?>" class="w-full bg-gray-50 border border-gray-200 text-gray-500 rounded-xl pl-10 pr-4 py-3 cursor-not-allowed select-none">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" readonly value="<?= esc($user['email']) ?>" class="w-full bg-gray-50 border border-gray-200 text-gray-500 rounded-xl pl-10 pr-4 py-3 cursor-not-allowed select-none">
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" readonly value="<?= esc($user['mobilenum']) ?>" class="w-full bg-gray-50 border border-gray-200 text-gray-500 rounded-xl pl-10 pr-4 py-3 cursor-not-allowed select-none">
                        </div>
                    </div>
            ...
            <div class="bg-white p-6 sm:p-10 rounded-2xl shadow-sm border border-gray-100 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php foreach($requirements as $req): ?>
                        <?php 
                            $is_optional_req6 = ($appType == 'New' && $req['id'] == 6); 
                        ?>
                        ...
                        <div class="relative">
                            ...
                            <div id="upload_box_<?= $req['id'] ?>" class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-emerald-500 transition bg-slate-50 relative group flex flex-col items-center justify-center min-h-[160px] w-full <?= $is_optional_req6 ? 'opacity-50 grayscale pointer-events-none' : '' ?>">
                                ...
                                <p class="font-bold text-gray-800 text-sm mb-1 leading-snug px-2"><?= esc($req['requirement_name']) ?> <span class="text-red-500 <?= $is_optional_req6 ? 'hidden' : '' ?>" id="asterisk_<?= $req['id'] ?>">*</span></p>
    ...
    <script>
        ...
        // --- Trigger Toasts from controller flash data ---
        <?php if (session()->getFlashdata('success')): ?>
            showToast("<?= esc(session()->getFlashdata('success')) ?>", "success");
            setTimeout(() => {
                window.top.location.href = '<?= base_url('/client/dashboard') ?>';
            }, 2000);
        <?php endif; ?>
