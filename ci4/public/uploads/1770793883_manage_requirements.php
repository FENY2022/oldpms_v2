...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    $new_status = $_POST['new_app_status'];
    $renew_status = $_POST['renewal_app_status'];
    $remove_file = isset($_POST['remove_file']) ? true : false;
    
    // Fetch old file to potentially delete it
    ...
    if ($new_file_path) {
        // Delete old file from server if replacing
        ...
        // Delete old file from server if explicitly removing via checkbox
        if ($oldFile && file_exists($oldFile)) { unlink($oldFile); }
        $stmt = $pdo->prepare("UPDATE requirements SET requirement_name=?, new_app_status=?, renewal_app_status=?, download_link=NULL WHERE id=?");
...
<body class="bg-gray-50 text-gray-800">
    ...
    <script>
        ...
        function openEditModal(data) {
            ...
            // Only show the "Remove File" checkbox if an attachment actually exists
